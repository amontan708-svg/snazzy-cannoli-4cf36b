# AGENTS.md — MinePrut.fun Architecture

This document is for AI agents and developers working on the MinePrut codebase.

## Project Overview

**MinePrut** is a streetwear e-commerce site built on TanStack Start and deployed on Netlify. It targets Moldova — prices are in MDL (Moldovan Leu), UI language is Romanian. The brand identity uses a dark theme (`#0a0a0a` background) with gold (`#c9a84c`) accents.

Pages: Home (products + quotes + reviews), About, Size Chart, The Math Behind (technical/math doc), Register (Netlify Forms), Product Detail (size picker, MDL price), Checkout success/cancel.

## Non-Obvious Decisions

- **`__root.tsx` includes `<Outlet />`** inside `RootDocument` — required for the header/footer to wrap all child pages
- **`marquee-track` animation** is defined in `styles.css` as a keyframe; text is duplicated 8× in JSX for seamless looping
- **EUR conversion** is hardcoded at 1 EUR = 19 MDL in `$productId.tsx` — update if rate changes significantly
- **Prices are in MDL** in the product data and UI; Stripe currently processes in USD (convert if needed)
- **`public/register-form.html`** is the static Netlify Forms skeleton — must mirror all field `name` attributes from `src/routes/register.tsx`
...
| Card | `#111` | Card/section backgrounds |
| Border | `#1a1a1a` / `#222` | Borders |
| Gold | `#c9a84c` | Primary accent, prices, CTA |
| Gold hover | `#e8c97a` | Button hover states |
| Muted | `#888` | Body text |
| Very muted | `#555` | Captions, metadata |
